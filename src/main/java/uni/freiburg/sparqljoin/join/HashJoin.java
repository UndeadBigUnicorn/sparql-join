...
public class HashJoin implements AbstractJoin {
    ...
    public ComplexTable probe(BuildOutput partitions, ComplexTable R, ComplexTable S,
        ...
        System.out.println("##### referenceTableDictionary " + referenceTableDictionary.getValues());
        System.out.println("##### probeTableDictionary " + probeTableDictionary.getValues());
            ...
            if (hashedReferenceTablePartitions.containsKey(hashedSKey)) {
                ...
                for (JoinedItems matchingReferenceTableItems : hashedReferenceTablePartitions.get(hashedSKey)) {
                    ...
                    if (referenceJoinKey == probeJoinKey) {
                        // No hash collision
                        System.out.println("##### MERGE key: " + referenceJoinKey);
                        System.out.println("##### MERGE before, r: " + referenceTableDictionary.getValues());
                        System.out.println("##### MERGE before, p: " + probeTableDictionary.getValues());
                        ...
                        System.out.println("##### MERGE after, r: " + referenceTableDictionary.getValues());
                        System.out.println("##### MERGE after, p: " + probeTableDictionary.getValues());

                        /*
TODO debug statements showing the problem - generated by testComplexParallelHashJoin:
##### referenceTableDictionary {1=LUKE, 2=HAN, 3=LEA}
##### probeTableDictionary {1=SKYWALKER, 2=SOLO, 3=ORGANA}
##### MERGE key: 0
##### MERGE before, r: {1=LUKE, 2=HAN, 3=LEA}
##### MERGE before, p: {1=SKYWALKER, 2=SOLO, 3=ORGANA}
##### MERGE after, r: {1=SKYWALKER, 2=HAN, 3=LEA}
##### MERGE after, p: {1=SKYWALKER, 2=SOLO, 3=ORGANA}
why is r[1] changed from LUKE to SKYWALKER?

##### MERGE key: 2
##### MERGE before, r: {1=SKYWALKER, 2=HAN, 3=LEA}
##### MERGE before, p: {1=SKYWALKER, 2=SOLO, 3=ORGANA}
##### MERGE after, r: {1=SKYWALKER, 2=SOLO, 3=LEA}
##### MERGE after, p: {1=SKYWALKER, 2=SOLO, 3=ORGANA}
TODO there is a collision (same hash value, different keys). why is HAN replaced by SOLO, instead of inserting a new entry for SOLO?
        ...
        System.out.println("##### join worker result " + referenceTableDictionary.getValues());
