...
public class JoinService {
     ...
     * @return               joined table
     */
    public VerticallyPartitionedTable hashJoin(VerticallyPartitionedTable R, VerticallyPartitionedTable S,
                                               String joinPropertyR, JoinOn joinOnR,
                                               String joinPropertyS, JoinOn joinOnS) {

        return join(new HashJoinOptimized(), R, S, joinPropertyR, joinOnR, joinPropertyS, joinOnS);
    }
     ...
     * @return               joined table
     */
    public VerticallyPartitionedTable parallelHashJoin(VerticallyPartitionedTable R, VerticallyPartitionedTable S,
                                         String joinPropertyR, JoinOn joinOnR,
                                         String joinPropertyS, JoinOn joinOnS) {
        return join(new ParallelHashJoinOptimized(), R, S, joinPropertyR, joinOnR, joinPropertyS, joinOnS);
    }
    ...
    private VerticallyPartitionedTable join(AbstractJoinOptimized joiner, VerticallyPartitionedTable R, VerticallyPartitionedTable S,
                              String joinPropertyR, JoinOn joinOnR,
                              String joinPropertyS, JoinOn joinOnS) {
        LOG.debug("Joining table '{}' on '{}'.{} = '{}'.{} ...",
                R.getProperties(), joinPropertyR, joinOnR, joinPropertyS, joinOnS);
        VerticallyPartitionedTable joinedTable = Performance.measure(() ->
                joiner.join(R, S, joinPropertyR, joinOnR, joinPropertyS,  joinOnS), String.format("%s", joiner.getClass().getSimpleName())
        );
        LOG.debug("Table 1 size: {}", R.propertyItems().get(joinPropertyR).getValues().size());
        LOG.debug("Table 2 size: {}", S.propertyItems().get(joinPropertyS).getValues().size());
        LOG.info("Joined table size: {}", joinedTable.propertyItems().get(joinPropertyR).getValues().size());
        return joinedTable;
    }
}
